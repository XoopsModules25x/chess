...
class ChessGame
{
    ...
    public $board;
    public $ac_move;   // for auto-completion of moves
    public $w_figures; // array of white's pieces
    public $b_figures; // array of black's pieces
    ...
    public function init_gamestate($fen = null)
    {
        ...
        } elseif (('w' == $this->gamestate['fen_active_color'] && $this->kingIsUnderAttack('b', 'w')) || ('b' == $this->gamestate['fen_active_color'] && $this->kingIsUnderAttack('w', 'b'))) {
        ...
        } elseif (('-' != $this->gamestate['fen_en_passant_target_square'] && 3 == $this->gamestate['fen_en_passant_target_square']{1} && 'b' != $this->gamestate['fen_active_color']) || ('-' != $this->gamestate['fen_en_passant_target_square']
                                                                                                                                                                                           && 6 == $this->gamestate['fen_en_passant_target_square']{1}
                                                                                                                                                                                           && 'w' != $this->gamestate['fen_active_color'])) {
            return _MD_CHESS_FENBAD_EP_COLOR; // en passant target square wrong color
        } elseif ('-' != $this->gamestate['fen_en_passant_target_square'] && 3 == $this->gamestate['fen_en_passant_target_square']{1} and 'wP' != $this->board[$this->boardCoordToIndex($this->gamestate['fen_en_passant_target_square']{0} . '4')]) {
            return _MD_CHESS_FENBAD_EP_NO_PAWN; // en passant target square for nonexistent pawn
        } elseif ('-' != $this->gamestate['fen_en_passant_target_square'] && 6 == $this->gamestate['fen_en_passant_target_square']{1} and 'bP' != $this->board[$this->boardCoordToIndex($this->gamestate['fen_en_passant_target_square']{0} . '5')]) {
            return _MD_CHESS_FENBAD_EP_NO_PAWN; // en passant target square for nonexistent pawn
        }
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $opp) {
                if (('P' == $this->board[$i]{1} && $this->checkPawnAttack($i, $dest_pos))
                    || ('P' != $this->board[$i]{1} && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos))) {
    ...
    public function isCheckMate($player, $opp)
    {
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $opp) {
                if (('P' == $this->board[$i]{1} && $this->checkPawnAttack($i, $king_pos))
                    || ('P' != $this->board[$i]{1} && $this->tileIsReachable($this->board[$i]{1}, $i, $king_pos))) {
                    $attackers[$count++] = $i;
                }
            }
        }
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $player) {
                if (('P' == $this->board[$i]{1} && $this->checkPawnAttack($i, $dest_pos))
                    || ('P' != $this->board[$i]{1} && 'K' != $this->board[$i]{1} && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos))
                    || ('K' == $this->board[$i]{1} && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos) && !$this->tileIsUnderAttack($opp, $dest_pos))) {
            ...
            for ($i = 0; $i < 64; $i++) {
                if ($this->board[$i]{0} == $player) {
                    if (('P' == $this->board[$i]{1} && $this->checkPawnMove($i, $pos))
                        || ('P' != $this->board[$i]{1} && 'K' != $this->board[$i]{1} && $this->tileIsReachable($this->board[$i]{1}, $i, $pos))) {
        ...
        if ($move[0] >= 'a' && $move[0] <= 'h') {
            ...
            elseif (1 == $fig_count) {
                ...
                elseif ($fig1_can_reach && $fig2_can_reach) {
                    ...
                    if (('-' == $action && 4 == strlen($move)) || ('x' == $action && 5 == strlen($move))) {
                        $hint = $move[1];
                    }
                    if (empty($hint)) {
    ...
    public function handleMove($move)
    {
        ...
        if ('O-O' == $move) {
            ...
            if ('P' == $fig_type) {
                if (('w' == $cur_player && $dest_pos >= 56) || ('b' == $cur_player && $dest_pos <= 7)) {
                    $pawn_upg = $move[strlen($move) - 1];
                    if ('?' == $pawn_upg) {
                        $pawn_upg     = 'Q';
