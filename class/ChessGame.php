...
class ChessGame
{
    ...
    public function init_gamestate($fen = null)
    {
        ...
        } elseif ($this->gamestate['fen_en_passant_target_square'] != '-' && $this->gamestate['fen_en_passant_target_square']{1} == 3 and $this->board[$this->boardCoordToIndex($this->gamestate['fen_en_passant_target_square']{0} . '4')] != 'wP') {
            return _MD_CHESS_FENBAD_EP_NO_PAWN; // en passant target square for nonexistent pawn
        } elseif ($this->gamestate['fen_en_passant_target_square'] != '-' && $this->gamestate['fen_en_passant_target_square']{1} == 6 and $this->board[$this->boardCoordToIndex($this->gamestate['fen_en_passant_target_square']{0} . '5')] != 'bP') {
            return _MD_CHESS_FENBAD_EP_NO_PAWN; // en passant target square for nonexistent pawn
        }
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $opp) {
                if (($this->board[$i]{1} == 'P' && $this->checkPawnAttack($i, $dest_pos))
                    || ($this->board[$i]{1} != 'P' && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos))) {
    ...
    public function isCheckMate($player, $opp)
    {
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $opp) {
                if (($this->board[$i]{1} == 'P' && $this->checkPawnAttack($i, $king_pos))
                    || ($this->board[$i]{1} != 'P' && $this->tileIsReachable($this->board[$i]{1}, $i, $king_pos))) {
                    $attackers[$count++] = $i;
                }
            }
        }
        ...
        for ($i = 0; $i < 64; $i++) {
            if ($this->board[$i]{0} == $player) {
                if (($this->board[$i]{1} == 'P' && $this->checkPawnAttack($i, $dest_pos))
                    || ($this->board[$i]{1} != 'P' && $this->board[$i]{1} != 'K' && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos))
                    || ($this->board[$i]{1} == 'K' && $this->tileIsReachable($this->board[$i]{1}, $i, $dest_pos) && !$this->tileIsUnderAttack($opp, $dest_pos))) {
            ...
            for ($i = 0; $i < 64; $i++) {
                if ($this->board[$i]{0} == $player) {
                    if (($this->board[$i]{1} == 'P' && $this->checkPawnMove($i, $pos))
                        || ($this->board[$i]{1} != 'P' && $this->board[$i]{1} != 'K' && $this->tileIsReachable($this->board[$i]{1}, $i, $pos))) {
        ...
        if ($move[0] >= 'a' && $move[0] <= 'h') {
            ...
            elseif ($fig_count == 1) {
                ...
                elseif ($fig1_can_reach && $fig2_can_reach) {
                    ...
                    if (($action == '-' && strlen($move) == 4) || ($action == 'x' && strlen($move) == 5)) {
                        $hint = $move[1];
                    }
                    if (empty($hint)) {
    ...
    public function handleMove($move)
    {
        ...
        if ($move == 'O-O') {
            ...
            if ($fig_type == 'P') {
                if (($cur_player == 'w' && $dest_pos >= 56) || ($cur_player == 'b' && $dest_pos <= 7)) {
                    $pawn_upg = $move[strlen($move) - 1];
                    if ($pawn_upg == '?') {
                        $pawn_upg     = 'Q';
